...
(start <- Sys.time())
system2(command = "aws", args = glue::glue("s3 sync noaa-goes16/ABI-L2-FDCC/ data/raw/goes16 --no-sign-request"), stdout = FALSE)
(difftime(Sys.time(), start, units = "mins"))

system2(command = "aws", args = glue::glue("s3 sync noaa-goes17/ABI-L2-FDCC/ data/raw/goes17 --no-sign-request"), stdout = FALSE)
(difftime(Sys.time(), start, units = "mins"))
